<template>
<div class="profile">
  <div class="overlay delete" v-if="deleteQuestion || addTicket">
 ...
 <div class="global-wrapper">
   <div class="profile-panel">
      <div class="profile-panel_menu">
        ...
        <button @click="addTicket = true">Создать заявку</button>
      </div>
      <div class="profile-panel_orders">
        <div class="order" v-for="(order, key) in filteredChoices" :key="key" :class="{ active: selectedOrder == order }" >
          ...
          <button v-if="order.status === 'Новая'" @click="deleteQuestion = true">Удалить заявку</button>
          <p class="option" v-else >Удаление заявки невозможно из-за её статуса</p>
        </div>
...
export default {
  ...
  methods: {
    createTicket(){
      ...
      if(this.newTicketInfo.adress.length < 5 || this.newTicketInfo.description.length < 5 || this.newTicketInfo.category.length < 5 || this.newTicketInfo.maxPrice.length < 5 || this.newTicketInfo.photo.length < 5){
